...
class Citizenos_Login_Form {
	...
	static public function register( $settings, $client_wrapper ){
		...
		add_shortcode( 'citizenos_login_button', array( $login_form, 'make_login_button' ) );
	...
	function make_login_button() {
		$text = apply_filters( 'citizenos-login-button-text', __( 'Login with Citizen OS' ) );
		...
		ob_start();
		?>
		<div class="citizenos-login-button" style="margin: 1em 0; text-align: center;">
